...
use crate::utils::{CustomColor, lerp_color};
...
pub fn time_to_string(miliseconds: u128) -> String {
    ...
    if ss >= 1.0 || mm > 0 {
        result.push_str(format!("{:.0}s", ss).as_str());
    } else {
...
pub fn bytes_to_string(number: u128) -> String {
    ...
    if number < 1024.0 {
        return format!("{:.0}K", number);
    }

    let number = round(number as f64 / 1024.0);

    if number < 1024.0 {
        return format!("{:.0}M", number);
    }

    format!("{:.0}G", round(number as f64 / 1024.0))
}
